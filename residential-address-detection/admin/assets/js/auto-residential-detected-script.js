jQuery(document).ready(function () {
  ...
  jQuery("#en_residential_addresses_template_start")
    .closest("tr")
    .addClass("en_residential_addresses_template_start_style");
  jQuery("#en_residential_addresses_template_start")
    .closest("table")
    .addClass("en_residential_addresses_template_table_start");

  jQuery(".en_woo_addons_always_include_residential_fee")
    .closest("tr")
    .addClass("en_woo_addons_always_include_residential_fee_style");
  jQuery("#residential_delivery_options_label_heading")
    .closest("tr")
    .addClass("residential_delivery_options_label_heading_style");
  jQuery(".en_default_unconfirmed_address_types_to")
    .closest("tr")
    .addClass("en_default_unconfirmed_address_types_to_style");
  jQuery(
    "#residential_delivery_options_label_description,#residential_delivery_options_addon_description,#en_default_unconfirmed_address_types_label_description"
  )
    .closest("tr")
    .addClass("residential_delivery_options_label_description_style");
  jQuery("#residential_delivery_options_addon_description")
    .closest("tr")
    .addClass("residential_delivery_options_addon_description_style");
  jQuery("#en_default_unconfirmed_address_types_label_description")
    .closest("tr")
    .addClass("residential_delivery_options_addon_description_style");
  jQuery("#suspend_automatic_detection_of_residential_addresses")
    .closest("tr")
    .addClass("suspend_automatic_detection_of_residential_addresses_style");
  jQuery("#auto_residential_delivery_plan_auto_renew")
    .closest("tr")
    .addClass("residential_delivery_options_plans_style");
  jQuery("#residential_delivery_current_subscription")
    .closest("tr")
    .addClass("residential_delivery_current_subscription_style");
  jQuery("#residential_delivery_current_usage")
    .closest("tr")
    .addClass("residential_delivery_current_usage_style");
  jQuery("#residential_delivery_plugin_name")
    .closest("tr")
    .addClass("residential_delivery_plugin_name_style");
  jQuery("#residential_delivery_subscription_status")
    .closest("tr")
    .addClass("residential_delivery_subscription_status_style");

  jQuery(".residential_delivery_options_label_description_style td").attr(
    "colspan",
    "2"
  );
  ...
  var en_need_suspended_rad = function (params = "", response = "") {
    ...
    var en_need_suspended_rad_content =
      "Residential address settings are being managed under the RAD (Residential Address Detection) page, which you can access in this plugin's navigation above. To use this setting, you must first deactivate or suspend the Residential Address Detection plugin";
    if (
      typeof response.en_need_suspended_rad != "undefined" &&
      response.en_need_suspended_rad == true
    ) {
      ...
      if (jQuery(".en_need_suspended_rad_content").length) {
        ...
        jQuery(".en_need_suspended_rad").after(
          "<span style='color: red;' class='en_need_suspended_rad_content'>" +
            en_need_suspended_rad_content +
            "</span>"
        );
      }
    } else {
  ...
  var en_woo_addons_popup_notifi_disabl_to_plan_hide = function () {
    ...
    return jQuery(".sm_notification_disable_to_plan_overlay").css({
      visibility: "hidden",
      opacity: "0",
    });
  };
  ...
  var en_woo_addons_popup_notifi_disabl_to_plan_show = function () {
    var selected_plan = jQuery("#auto_residential_delivery_plan_auto_renew")
      .find("option:selected")
      .text();
    jQuery(".sm_notification_disable_to_plan_overlay")
      .last()
      .find("#selected_plan_popup")
      .text(selected_plan);
  ...
  jQuery(".cancel_plan").on("click", function () {
    ...
    jQuery("#auto_residential_delivery_plan_auto_renew").prop(
      "selectedIndex",
      0
    );
    return false;
  ...
  jQuery(".confirm_plan").on("click", function () {
    var params = "";
    en_woo_addons_popup_notifi_disabl_to_plan_hide();
    var monthly_pckg = jQuery(
      "#auto_residential_delivery_plan_auto_renew"
    ).val();
    var plugin_name = jQuery("#residential_delivery_plugin_name").attr(
      "placeholder"
    );
    var data = {
      plugin_name: plugin_name,
      selected_plan: monthly_pckg,
      action: "en_woo_addons_upgrade_plan_submit",
    };
    params = {
  ...
  var monthly_packg_response = function (params, response) {
    ...
    if (parsRes.severity == "SUCCESS") {
      jQuery(
        "#auto_residential_delivery_plan_auto_renew option[value='TR']"
      ).remove();

      if (
        typeof parsRes.subscription_packages_response != "undefined" &&
        parsRes.subscription_packages_response == "yes"
      ) {
        jQuery("#residential_delivery_current_subscription")
          .next(".description")
        ...
        jQuery("#residential_delivery_subscription_status").attr(
          "placeholder",
          "yes"
        );
        jQuery("#suspend_automatic_detection_of_residential_addresses").prop(
          "disabled",
          false
        );
      }
      if (
        ...
        if (jQuery(".en_woo_addons_always_include_residential_fee").length) {
          jQuery(".en_woo_addons_always_include_residential_fee")
            .closest("tr")
            .after(
              ' <tr><td colspan="2"><div class="alert-plan-messages alert-success auto_resid_package_msg">  ' +
                params.message_ph +
                " </div> </td> </tr>"
            );
        } else {
          jQuery(".residential_delivery_options_label_heading_style").after(
            ' <tr><td colspan="2"><div class="alert-plan-messages alert-success auto_resid_package_msg">  ' +
              params.message_ph +
              " </div> </td> </tr>"
          );
      ...
      if (jQuery(".en_woo_addons_always_include_residential_fee").length) {
        jQuery(".en_woo_addons_always_include_residential_fee")
          .closest("tr")
          .after(
            ' <tr><td colspan="2"><div class="alert-plan-messages alert-danger auto_resid_package_msg">  ' +
              parsRes.Message +
              " </div> </td> </tr>"
          );
      } else {
  ...
  var en_woo_addons_monthly_packg = function (monthly_pckg) {
    ...
    if (window.existing_plan == "disable") {
      ...
      params = {
        ...
        message_ph:
          " You have disabled the Residential Address Detection plugin. The plugin will stop working when the current plan is depleted or expires.",
      };
    } else {
      params = {
        loading_id: "auto_residential_delivery_plan_auto_renew",
  ...
  var suspend_automatic_detection = function (params = "", response = "") {
    ...
    var subscription_status = jQuery(
      "#residential_delivery_subscription_status"
    ).attr("placeholder");
    $subscription_status = "yes";
    if (subscription_status == "yes") {
      //   jQuery(".residential_delivery_options_plans_style th label").text(
      //     "Auto-renew"
      //   );
      if (suspend_automatic) {
        jQuery(".en_woo_addons_always_include_residential_fee").prop(
          "disabled",
          false
        );
        ...
        typeof params.loading_id != "undefined" && params.loading_id.length > 0
          ? jQuery("#" + params.loading_id).css(
              ...
              "rgba(255, 255, 255, 1) url(" +
                plugins_url +
                "/residential-address-detection/admin/assets/images/processing.gif) no-repeat scroll 50% 50%"
            )
          : "";
   ...
   */
  jQuery("#auto_residential_delivery_plan_auto_renew").on(
    "change",
    function () {
  ...
  var suspend_automatic_detection_disabled = function () {
    var always_include_residential_ind = jQuery(
      ".en_woo_addons_always_include_residential_fee"
    ).attr("id");
    return {
   ...
   * Suspend plan of RAD.
   */
  jQuery("#suspend_automatic_detection_of_residential_addresses").on(
    "click",
    function () {
      var data = "";
      var params = "";
  ...
  jQuery(".en_default_unconfirmed_address_types_to").on("click", function () {
    ...
    jQuery(this)
      .closest("label")
      .after(
        "<span class='en-default-unconfirmed-address-change'>Saved</span>"
      );
    setTimeout(function () {
