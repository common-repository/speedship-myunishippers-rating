...
add_action('wp_ajax_speed_en_wwe_ltl_activate_hit_to_update_plan', 'speed_en_wwe_ltl_activate_hit_to_update_plan');
add_action('wp_ajax_nopriv_speed_en_wwe_ltl_activate_hit_to_update_plan', 'speed_en_wwe_ltl_activate_hit_to_update_plan');

/**
 * Activate WWE LTL
 */
function speed_en_wwe_ltl_activate_hit_to_update_plan()
{
    ...
    $url = SPEED_WWE_FREIGHT_DOMAIN_HITTING_URL . "/web-hooks/subscription-plans/create-plugin-webhook.php?";
    ...
    speed_en_check_wwe_ltl_plan_on_product_detail();
}

/**
 * Product detail Features
 */
function speed_en_check_wwe_ltl_plan_on_product_detail()
{
    ...
    //  Nested material
    $nested_mateials = apply_filters('speed_wwe_quests_quotes_plans_suscription_and_features', 'nested_material');
    ...
    if (get_option('wwe_quests_store_type') == "1") {
        $action_dropship = apply_filters('speed_wwe_quests_quotes_plans_suscription_and_features', 'multi_dropship');
    ...
    //  Insurance Fee
    $action_insurance = apply_filters('speed_wwe_quests_quotes_plans_suscription_and_features', 'insurance_fee');
 ...
 * Deactivate WWE LTL
 */
function speed_en_wwe_ltl_deactivate_hit_to_update_plan()
{
 ...
 * @return string
 */
function speed_en_wwe_ltl_plan_name()
{
 ...
 * @return string
 */
function speed_en_wwe_ltl_plan_notice()
{
    if (isset($_GET['tab']) && ($_GET['tab'] == "wwe_quests")) {
        $plan_package = speed_en_wwe_ltl_plan_name();
        ...
        if ($store_type == "1" || $store_type == "0" && ($plan_number == "0" || $plan_number == "1" || $plan_number == "2" || $plan_number == "3")) {
            ...
            $click_here_to_update_plan = ' <a href="javascript:void(0)" data-action="speed_en_wwe_ltl_activate_hit_to_update_plan" onclick="en_update_plan(this);">Click here</a> to refresh the plan';
...
add_action('admin_notices', 'speed_en_wwe_ltl_plan_notice');
