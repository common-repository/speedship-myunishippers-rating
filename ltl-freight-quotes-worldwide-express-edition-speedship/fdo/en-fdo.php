...
class SpeedEnSpeedfreightFdo
{
    ...
    public function en_cart_package($package)
    {
        ...
        $this->en_fdo_meta_data['plugin_name'] = 'wwe_quests';
        $accessorials['residential'] = get_option('wc_settings_wwe_residential_delivery') == 'yes' ? true : false;
        $accessorials['liftgate'] = get_option('wc_settings_wwe_lift_gate_delivery') == 'yes' ? true : false;
    ...
    public function en_package_items($items)
    {
        ...
        foreach ($items as $item_key => $item_data) {
            $productId = $productName = $productQty = $actualProductPrice = $products = $productPrice = $productWeight = $productLength = $productWidth = $productHeight = $ptype = $hazardousMaterial = $productType = $productSku = $productClass = $attributes = $variantId = '';
            extract($item_data);
