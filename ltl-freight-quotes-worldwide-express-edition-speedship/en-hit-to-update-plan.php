<?php
$get_option = get_option('wwe_quests_web_hook_plan_requests');
$en_web_hook_requests = (isset($get_option) && (!empty($get_option))) ? json_decode($get_option, TRUE) : [];
$en_web_hook_requests[] = (isset($_GET)) ? $_GET : [];
update_option('wwe_quests_web_hook_plan_requests', json_encode($en_web_hook_requests));
...
if ($plan == "0" || $plan == "1" || $plan == "2" || $plan == "3") {
    ...
    update_option('wwe_ltl_package_expire_days', "$expire_days");
