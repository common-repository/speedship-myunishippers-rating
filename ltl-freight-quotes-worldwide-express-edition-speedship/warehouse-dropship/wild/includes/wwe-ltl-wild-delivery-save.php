...
if (!class_exists("EnWweLTLWooWdAddonsAjaxReqIncludes")) {
    ...
    class EnWweLTLWooWdAddonsAjaxReqIncludes
    {
        ...
        {
            add_action('wp_ajax_nopriv_en_wd_get_address', array($this, 'speed_get_address_api_ajax'));
            add_action('wp_ajax_en_wd_get_address', array($this, 'speed_get_address_api_ajax'));
         ...
         */
        function speed_get_address_api_ajax()
        {
            if (isset($_POST['origin_zip'])) {
                $speed_map_address = (isset($_POST['origin_zip'])) ? sanitize_text_field($_POST['origin_zip']) : "";
                $zipCode = str_replace(' ', '', $speed_map_address);
                $accessLevel = 'address';
                $Speed_Get_ltl_distance = new Speed_Get_ltl_distance();
                $resp_json = $Speed_Get_ltl_distance->ltl_get_distance($zipCode, $accessLevel);
        ...
        function wwe_ltl_save_warehouse_ajax()
        {
            ...
            $origin_country = 'US';
            if (isset($_POST['origin_country']) && sanitize_text_field($_POST['origin_country']) != '' && strtolower(sanitize_text_field($_POST['origin_country'])) != 'un') {
            ...
            $input_data_arr = array(
                ...
                'address' => sanitize_text_field($_POST['origin_address']),
                'zip' => sanitize_text_field($_POST['origin_zip']),
                'country' => $origin_country,
                'location' => sanitize_text_field($_POST['location']),
                'enable_store_pickup' => (sanitize_text_field($_POST['enable_instore']) === 'true') ? 1 : 0,
                'miles_store_pickup' => sanitize_text_field($_POST['address_miles_instore']),
                'match_postal_store_pickup' => sanitize_text_field($_POST['zipmatch_instore']),
                'checkout_desc_store_pickup' => sanitize_text_field($_POST['desc_instore']),
                // Terminal phone number
                'phone_instore' => sanitize_text_field($_POST['phone_instore']),
                'enable_local_delivery' => (sanitize_text_field($_POST['enable_delivery']) === 'true') ? 1 : 0,
                'miles_local_delivery' => sanitize_text_field($_POST['address_miles_delivery']),
                'match_postal_local_delivery' => sanitize_text_field($_POST['zipmatch_delivery']),
                'checkout_desc_local_delivery' => sanitize_text_field($_POST['desc_delivery']),
                'fee_local_delivery' => sanitize_text_field($_POST['fee_delivery']),
                'suppress_local_delivery' => (sanitize_text_field($_POST['supppress_delivery']) === 'true') ? 1 : 0,
            );
            ...
            if ($validateData["city"] != 'Error') {
                ...
                if (isset($validateData["city"])) {
                    $get_warehouse_id = (isset($_POST['origin_id']) && intval($_POST['origin_id'])) ? sanitize_text_field($_POST['origin_id']) : "";
                ...
                $address['senderCity'] = sanitize_text_field($_POST['origin_city']);
                $address['senderState'] = sanitize_text_field($_POST['origin_state']);
                $address['senderZip'] = sanitize_text_field($_POST['origin_zip']);
                $address['senderCountryCode'] = $origin_country;
                $addresses[] = $address;
                $addresses['update_status'] = $update_status;
                speed_wwe_ltl_get_all_warehouse_dropship($addresses);
        ...
        function wwe_ltl_edit_warehouse_ajax()
        {
            // Terminal phone number
            global $wpdb;
            $get_warehouse_id = (isset($_POST['edit_id']) && intval($_POST['edit_id'])) ? sanitize_text_field($_POST['edit_id']) : "";
        ...
        {
            global $wpdb;
            $get_warehouse_id = (isset($_POST['delete_id']) && intval($_POST['delete_id'])) ? sanitize_text_field($_POST['delete_id']) : "";
        ...
        function wwe_ltl_save_dropship_ajax()
        {
            ...
            $insert_qry = $update_qry = '';
            $dropship_country = 'US';
            if (isset($_POST['dropship_country']) && sanitize_text_field($_POST['dropship_country']) != '' && strtolower(sanitize_text_field($_POST['dropship_country'])) != 'un') {
                $dropship_country =  sanitize_text_field($_POST['dropship_country']);
            }
            ...
            $input_data_arr = array(
                ...
                // Origin terminal address
                'address' => sanitize_text_field($_POST['dropship_address']),
                'zip' => sanitize_text_field($_POST['dropship_zip']),
                'country' => $dropship_country,
                'location' => sanitize_text_field($_POST['location']),
                'nickname' => sanitize_text_field($_POST['nickname']),
                'enable_store_pickup' => ($_POST['enable_instore'] === 'true') ? 1 : 0,
                'miles_store_pickup' => sanitize_text_field($_POST['address_miles_instore']),
                'match_postal_store_pickup' => sanitize_text_field($_POST['zipmatch_instore']),
                'checkout_desc_store_pickup' => sanitize_text_field($_POST['desc_instore']),
                // Terminal phone number
                'phone_instore' => sanitize_text_field($_POST['phone_instore']),
                'enable_local_delivery' => ($_POST['enable_delivery'] === 'true') ? 1 : 0,
                'miles_local_delivery' => sanitize_text_field($_POST['address_miles_delivery']),
                'match_postal_local_delivery' => sanitize_text_field($_POST['zipmatch_delivery']),
                'checkout_desc_local_delivery' => sanitize_text_field($_POST['desc_delivery']),
                'fee_local_delivery' => sanitize_text_field($_POST['fee_delivery']),
                'suppress_local_delivery' => ($_POST['supppress_delivery'] === 'true') ? 1 : 0,
            );
            $validateData = $this->pkg_validate_post_data($input_data_arr);
            ...
            if ($validateData["city"] != 'Error' && $validateData["nickname"] != 'Error') {
                ...
                if (isset($validateData["city"])) {
                    $get_dropship_id = (isset($_POST['dropship_id']) && intval($_POST['dropship_id'])) ? sanitize_text_field($_POST['dropship_id']) : "";
                ...
                $address['senderCity'] = sanitize_text_field($_POST['dropship_city']);
                $address['senderState'] = sanitize_text_field($_POST['dropship_state']);
                $address['senderZip'] = sanitize_text_field($_POST['dropship_zip']);
                $address['senderCountryCode'] = $dropship_country;
                $addresses[] = $address;
                $addresses['update_status'] = $update_status;
                speed_wwe_ltl_get_all_warehouse_dropship($addresses);
        ...
        function wwe_ltl_edit_dropship_ajax()
        {
            // Terminal phone number
            global $wpdb;
            $get_dropship_id = (isset($_POST['dropship_edit_id']) && intval($_POST['dropship_edit_id'])) ? sanitize_text_field($_POST['dropship_edit_id']) : "";
            $warehous_list = $wpdb->get_results(
        ...
        function wwe_ltl_delete_dropship_ajax()
        {
            global $wpdb;
            $get_dropship_id = '';
            $dropship_id = (isset($_POST['dropship_delete_id']) && intval($_POST['dropship_delete_id'])) ? sanitize_text_field($_POST['dropship_delete_id']) : "";
