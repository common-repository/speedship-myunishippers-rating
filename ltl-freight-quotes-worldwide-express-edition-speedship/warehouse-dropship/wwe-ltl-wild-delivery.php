...
require_once 'wild/includes/wwe-ltl-wild-delivery-save.php';


/**
 * Admin Warehouse Dropship Scripts
 */
if (!function_exists("en_wwe_ltl_woo_wd_admin_script_style")) {
    function en_wwe_ltl_woo_wd_admin_script_style()
    {
        wp_enqueue_script('en_woo_wd_tagging', plugin_dir_url(__FILE__) . '/wild/assets/js/tagging.js', [], '1.0.0');
        wp_localize_script('en_woo_wd_tagging', 'script', array(
            'pluginsUrl' => plugins_url(),
        ));

        wp_enqueue_script('en_wwe_ltl_woo_wd_script', plugin_dir_url(__FILE__) . '/wild/assets/js/wwe_ltl_warehouse_section.js', [], '1.0.3');
        wp_localize_script('en_wwe_ltl_woo_wd_script', 'script', array(
            'pluginsUrl' => plugins_url(),
        ));

        wp_register_style('warehouse_section', plugin_dir_url(__FILE__) . '/wild/assets/css/warehouse_section.css', false, '1.0.2');
        wp_enqueue_style('warehouse_section');
    }

    add_action('admin_enqueue_scripts', 'en_wwe_ltl_woo_wd_admin_script_style');
}

/**
 * Warehouse Template
 */
if (!function_exists('wwe_ltl_warehouse_template')) {
    function wwe_ltl_warehouse_template($action = FALSE)
    {
...
?>
                ...
                <tbody>
                    ...
                    ?>
                            <tr class="<?php echo (strlen($tr_disabled_me) > 0 && $count != 0) ? $tr_disabled_me : ""; ?>" id="row_<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>" data-id="<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>">
                                ...
                                <td class="en_wd_warehouse_list_data">
                                    <a href="javascript(0)" onclick="return wwe_ltl_en_wd_edit_warehouse(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img src="<?php echo plugins_url('wild/assets/images/edit.png', __FILE__); ?>" title="Edit"></a>
                                    <a href="javascript(0)" onclick="return wwe_ltl_en_wd_delete_current_warehouse(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img src="<?php echo plugins_url('wild/assets/images/delete.png', __FILE__); ?>" title="Delete"></a>
                                </td>
                            </tr>
                        <?php
                            $count++;
     ...
     * Dropship Template
     */
    if (!function_exists('wwe_ltl_dropship_template')) {
        function wwe_ltl_dropship_template($action = FALSE)
        {
                    ...
                    <tbody>
                        ...
                        ?>
                                <tr class="<?php echo (strlen($tr_disabled_me) > 0 && $count != 0) ? $tr_disabled_me : ""; ?>" id="row_<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>">
                                    ...
                                    <td class="en_wd_dropship_list_data">
                                        <a href="javascript(0)" onclick="return wwe_ltl_en_wd_edit_dropship(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img src="<?php echo plugins_url('wild/assets/images/edit.png', __FILE__); ?>" title="Edit"></a>
                                        <a href="javascript(0)" onclick="return wwe_ltl_en_wd_delete_current_dropship(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img src="<?php echo plugins_url('wild/assets/images/delete.png', __FILE__); ?>" title="Delete"></a>
                                    </td>
                                </tr>
                            <?php
                                $count++;
