...
add_action('wp_ajax_sm_get_address', 'speed_speed_warehouse_get_address_api_ajax');
add_action('wp_ajax_nopriv_sm_get_address', 'speed_speed_warehouse_get_address_api_ajax');

/**
 * Get Address From ZipCode Using API
 */
function speed_speed_warehouse_get_address_api_ajax()
{
 ...
 * @return string
 */
function speedsmpkgValidatePostData($sPostData)
{
 ...
 * @return array
 */
function speed_wwe_small_filtered_data($validateData)
{
...
add_action('wp_ajax_sm_save_warehouse', 'speed_save_warehouse_ajax');
add_action('wp_ajax_nopriv_sm_save_warehouse', 'speed_save_warehouse_ajax');

/**
 * Save Warehouse Function
 * @global $wpdb
 */
function speed_save_warehouse_ajax()
{

    global $wpdb;
    $input_data_arr = array(
        'speed_city' => sanitize_text_field($_POST['origin_city']),
        'speed_state' => sanitize_text_field($_POST['origin_state']),
        'speed_zip' => sanitize_text_field($_POST['origin_zip']),
        'country' => sanitize_text_field($_POST['origin_country']),
        'location' => sanitize_text_field($_POST['location']),
    );

    $validateData = speedsmpkgValidatePostData($input_data_arr);
    $get_warehouse = $wpdb->get_results(
        "SELECT * FROM " . $wpdb->prefix . "warehouse WHERE city = '" . $validateData["speed_city"] . "' && state = '" . $validateData["speed_state"] . "' && zip = '" . $validateData["speed_zip"] . "'"
    );
    if ($validateData["speed_city"] != 'Error') {
        $data = speed_wwe_small_filtered_data($validateData);
        if (isset($validateData["speed_city"])) {
            $speed_get_warehouse_id = (isset($_POST['origin_id']) && intval($_POST['origin_id'])) ? sanitize_text_field($_POST['origin_id']) : "";
            if ($speed_get_warehouse_id && empty($get_warehouse)) {
                $update_qry = $wpdb->update(
                    $wpdb->prefix . 'warehouse',
                    $data,
                    array('id' => $speed_get_warehouse_id)
                );
            } else {
...
add_action('wp_ajax_sm_edit_warehouse', 'speed_edit_warehouse_ajax');
add_action('wp_ajax_nopriv_sm_edit_warehouse', 'speed_edit_warehouse_ajax');

/**
 * Edit Warehouse Function
 * @global $wpdb
 */
function speed_edit_warehouse_ajax()
{

    global $wpdb;
    $speed_get_warehouse_id = (isset($_POST['edit_id']) && intval($_POST['edit_id'])) ? sanitize_text_field($_POST['edit_id']) : "";
    $warehous_list = $wpdb->get_results(
        "SELECT * FROM " . $wpdb->prefix . "warehouse WHERE id=$speed_get_warehouse_id"
    );
    echo json_encode($warehous_list);
    exit;
}
...
function delete_warehouse_ajax()
{
    ...
    global $wpdb;
    $speed_get_warehouse_id = (isset($_POST['delete_id']) && intval($_POST['delete_id'])) ? sanitize_text_field($_POST['delete_id']) : "";
    $qry = $wpdb->delete($wpdb->prefix . 'warehouse', array('id' => $speed_get_warehouse_id, 'location' => 'warehouse'));
    echo esc_html($qry);
    exit;
}
