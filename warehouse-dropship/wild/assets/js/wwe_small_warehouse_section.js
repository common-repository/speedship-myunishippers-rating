jQuery(document).ready(function () {
  ...
  jQuery("#en_wd_origin_address,#en_wd_dropship_address").keypress(function (
    e
  ) {
    if (!String.fromCharCode(e.keyCode).match(/^[a-z\d\-_,.\s]+$/i))
      return false;
  });
  ...
  // Terminal phone number
  jQuery(
    "#en-phone-number, #en_wd_origin_address, #en_wd_origin_zip , #en_wd_origin_city , #en_wd_origin_state , #en_wd_origin_country, #instore-pickup-address, #instore-pickup-desc, #local-delivery-address, #local-delivery-desc, #local-delivery-fee"
  ).focus(function (e) {
  ...
  // Terminal phone number
  jQuery(
    "#en-phone-number, #en_wd_dropship_nickname, #en_wd_dropship_address, #en_wd_dropship_zip , #en_wd_dropship_city , #en_wd_dropship_state, #en_wd_dropship_country, #instore-pickup-address, #instore-pickup-desc, #local-delivery-address, #local-delivery-desc, #local-delivery-fee"
  ).focus(function (e) {
    jQuery("#add_dropships #" + this.id).css({ "border-color": "#ddd" });
  });

  jQuery("._en_hazardous_material")
    .closest("p")
    .addClass("_en_hazardous_material");
  ...
  jQuery(
    "#instore-pickup-address , #local-delivery-address , #local-delivery-fee"
  ).keydown(function (e) {
...
if (typeof reset_warehouse_popup_form != "function") {
  function reset_warehouse_popup_form() {
    ...
    jQuery("#add_warehouses")
      .find("input[type='checkbox']")
      .prop("checked", false);
    jQuery(
      "#instore-pickup-zipmatch .tag-i, #local-delivery-zipmatch .tag-i"
    ).trigger("click");
  }
}
...
if (typeof localDeliveryInputVal != "function") {
  function localDeliveryInputVal(formID) {
    ...
    var localDelivery = {
      enable_delivery: jQuery(formID + " #enable-local-delivery").is(
        ":checked"
      ),
      ...
      supppress_delivery: jQuery(formID + " #suppress-local-delivery").is(
        ":checked"
      ),
    };
...
if (typeof load_inside_pikup_and_local_delivery_data != "function") {
  function load_inside_pikup_and_local_delivery_data(data, formID) {
    jQuery(
      "" +
        formID +
        " #instore-pickup-zipmatch .tag-i, " +
        formID +
        " #local-delivery-zipmatch .tag-i"
    ).trigger("click");

    data[0].enable_store_pickup == 1
      ? jQuery(formID + " #enable-instore-pickup").prop("checked", true)
      : "";
    data[0].enable_local_delivery == 1
      ? jQuery(formID + " #enable-local-delivery").prop("checked", true)
      : "";
    data[0].suppress_local_delivery == 1
      ? jQuery(formID + " #suppress-local-delivery").prop("checked", true)
      : "";

    jQuery(formID + " #instore-pickup-desc").val(
      data[0].checkout_desc_store_pickup
    );
    // Terminal phone number
    jQuery(formID + " #en-phone-number").val(data[0].phone_instore);
    jQuery(formID + " #local-delivery-desc").val(
      data[0].checkout_desc_local_delivery
    );
    ...
    jQuery.each(storePickupZipArray, function (index, value) {
      jQuery(formID + " #instore-pickup-zipmatch .type-zone")
        .val(value)
        .focusout();
    });
    ...
    jQuery.each(localDeliveryZipArray, function (index, value) {
      jQuery(formID + " #local-delivery-zipmatch .type-zone")
        .val(value)
        .focusout();
    });

    jQuery(formID + " #instore-pickup-address").val(data[0].miles_store_pickup);
    jQuery(formID + " #local-delivery-address").val(
      data[0].miles_local_delivery
    );
  }
}
...
if (typeof en_wwe_small_wd_save_warehouse != "function") {
  function en_wwe_small_wd_save_warehouse() {
    ...
    var enable_local_delivery = jQuery(
      "#add_warehouses #enable-local-delivery"
    ).is(":checked");
    var local_delivary_fee = jQuery(
      "#add_warehouses #local-delivery-fee"
    ).val();

    var warehouse_en_wd_origin_zip = jQuery(
      "#add_warehouses #en_wd_origin_zip"
    ).val();
    var warehouse_en_wd_origin_state = jQuery(
      "#add_warehouses #en_wd_origin_state"
    ).val();
    var warehouse_en_wd_origin_country = jQuery(
      "#add_warehouses #en_wd_origin_country"
    ).val();
    var warehouse_en_wd_origin_city = jQuery(
      "#add_warehouses #en_wd_origin_city"
    ).val();
    ...
    switch (true) {
      ...
      case enable_local_delivery && local_delivary_fee.length <= 0:
        jQuery("#add_warehouses #local-delivery-fee")
          .parent()
          .find(".en_wd_err")
          .html("Local delivery fee is required.");
        jQuery("#add_warehouses #local-delivery-fee").css({
          "border-color": "#e81123",
        });
        return false;

      case !/^[a-zA-Z0-9\-\s]+$/.test(warehouse_en_wd_origin_zip):
        jQuery("#add_warehouses #en_wd_origin_zip")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid zip code.");
        jQuery("#add_warehouses #en_wd_origin_zip").css({
          "border-color": "#e81123",
        });
        return false;
      case !/^[a-zA-Z0-9\-\s]+$/.test(warehouse_en_wd_origin_state):
        jQuery("#add_warehouses #en_wd_origin_state")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid State.");
        jQuery("#add_warehouses #en_wd_origin_state").css({
          "border-color": "#e81123",
        });
        return false;
      case !/^[a-zA-Z0-9\-\s]+$/.test(warehouse_en_wd_origin_country):
        jQuery("#add_warehouses #en_wd_origin_country")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid Country.");
        jQuery("#add_warehouses #en_wd_origin_country").css({
          "border-color": "#e81123",
        });
        return false;
      case !/^[a-zA-Z0-9\-\s]+$/.test(warehouse_en_wd_origin_city):
        jQuery("#add_warehouses #en_wd_origin_city")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid City.");
        jQuery("#add_warehouses #en_wd_origin_city").css({
          "border-color": "#e81123",
        });
        return false;
    }
    ...
    var arrObj = [
      postForm,
      instorePickupInputVal(form_id),
      localDeliveryInputVal(form_id),
    ];
    ...
    jQuery.ajax({
      ...
      dataType: "json",
      beforeSend: function () {
        jQuery(".wwe_small_save_warehouse_form")
      ...
      success: function (data) {
        ...
        if (data.insert_qry == 1) {
          ...
          jQuery("tr[id=row_" + WarehpuseDataId + "]").html(
            '<td class="en_wd_warehouse_list_data">' +
              data.origin_city +
              '</td><td class="en_wd_warehouse_list_data">' +
              data.origin_state +
              '</td><td class="en_wd_warehouse_list_data">' +
              data.origin_zip +
              '</td><td class="en_wd_warehouse_list_data">' +
              data.origin_country +
              '</td><td class="en_wd_warehouse_list_data"><a href="javascript(0)" title="Edit" onclick="return en_wwe_small_wd_edit_warehouse(' +
              WarehpuseDataId +
              ')"><img src="' +
              script.pluginsUrl +
              '/speedship-myunishippers-rating/warehouse-dropship/wild/assets/images/edit.png"></a><a href="javascript(0)" title="Delete" onclick="return en_wwe_small_wd_delete_current_warehouse(' +
              WarehpuseDataId +
              ');"><img src="' +
              script.pluginsUrl +
              '/speedship-myunishippers-rating/warehouse-dropship/wild/assets/images/delete.png"></a></td>'
          );
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        if (data[0]) {
          ...
          window.location.href = jQuery(".en_wd_add_warehouse_btn").attr(
            "href"
          );
...
if (typeof en_wwe_small_wd_save_dropship != "function") {
  function en_wwe_small_wd_save_dropship() {
    ...
    var enable_local_delivery = jQuery(
      "#add_dropships  #enable-local-delivery"
    ).is(":checked");
    var dropship_en_wd_dropship_country = jQuery(
      "#add_dropships #en_wd_dropship_country"
    ).val();
    var dropship_en_wd_dropship_nickname = jQuery(
      "#add_dropships #en_wd_dropship_nickname"
    ).val();

    var dropship_local_delivary_fee = jQuery(
      "#add_dropships #local-delivery-fee"
    ).val();
    var dropship_en_wd_dropship_state = jQuery(
      "#add_dropships #en_wd_dropship_state"
    ).val();
    var dropship_en_wd_dropship_zip = jQuery(
      "#add_dropships #en_wd_dropship_zip"
    ).val();
    var dropship_en_wd_dropship_city = jQuery(
      "#add_dropships #en_wd_dropship_city"
    ).val();
    ...
    switch (true) {
      ...
      case enable_local_delivery && dropship_local_delivary_fee.length <= 0:
        jQuery("#add_dropships #local-delivery-fee")
          .parent()
          .find(".en_wd_err")
          .html("Local delivery fee is required.");
        jQuery("#add_dropships #local-delivery-fee").css({
          "border-color": "#e81123",
        });
        return false;
      ...
      case dropship_en_wd_dropship_nickname.length > 20:
        ...
        jQuery(".dynamic_dropship_error p").html(
          "<strong>Error!</strong> Nickname should be less than 20 characters."
        );
        jQuery("#add_dropships #en_wd_dropship_nickname").css({
          "border-color": "#e81123",
        });
        jQuery(".content").delay(200).animate({ scrollTop: 0 }, 300);
        setTimeout(function () {
      ...
      case !/^[a-zA-Z0-9 ]*$/.test(dropship_en_wd_dropship_nickname):
        jQuery(".dynamic_dropship_error").show("slow");
        jQuery(".dynamic_dropship_error p").html(
          "<strong>Error!</strong> Nickname should not contain any special characters."
        );
        jQuery("#add_dropships #en_wd_dropship_nickname").css({
          "border-color": "#e81123",
        });
      ...
      case !/^[a-zA-Z0-9\-\s]+$/.test(dropship_en_wd_dropship_zip):
        jQuery("#add_dropships #en_wd_dropship_zip")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid zip code.");
        jQuery("#add_dropships #en_wd_dropship_zip").css({
          "border-color": "#e81123",
        });
        return false;
      case !/^[a-zA-Z0-9\-\s]+$/.test(dropship_en_wd_dropship_state):
        jQuery("#add_dropships #en_wd_dropship_state")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid State.");
        jQuery("#add_dropships #en_wd_dropship_state").css({
          "border-color": "#e81123",
        });
        return false;
      case !/^[a-zA-Z0-9\-\s]+$/.test(dropship_en_wd_dropship_country):
        jQuery("#add_dropships #en_wd_dropship_country")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid Country.");
        jQuery("#add_dropships #en_wd_dropship_country").css({
          "border-color": "#e81123",
        });
        return false;
      case !/^[a-zA-Z0-9\-\s]+$/.test(dropship_en_wd_dropship_city):
        jQuery("#add_dropships #en_wd_dropship_city")
          .parent()
          .find(".en_wd_err")
          .html("Please! Enter valid City.");
        jQuery("#add_dropships #en_wd_dropship_city").css({
          "border-color": "#e81123",
        });
        return false;
    }
        ...
        if (data.insert_qry == 1) {
          ...
          jQuery("tr[id=row_" + WarehpuseDataId + "]").html(
            '<td class="en_wd_dropship_list_data">' +
              data.nickname +
              '</td><td class="en_wd_dropship_list_data">' +
              data.origin_city +
              '</td><td class="en_wd_dropship_list_data">' +
              data.origin_state +
              '</td><td class="en_wd_dropship_list_data">' +
              ...
              '</td><td class="en_wd_dropship_list_data">' +
              data.origin_country +
              '</td><td class="en_wd_dropship_list_data"><a href="javascript(0)" title="Edit" onclick="return en_wwe_small_wd_edit_dropship(' +
              WarehpuseDataId +
              ')"><img src="' +
              script.pluginsUrl +
              '/speedship-myunishippers-rating/warehouse-dropship/wild/assets/images/edit.png"></a><a href="javascript(0)" title="Delete" onclick="return en_wwe_small_wd_delete_current_dropship(' +
              WarehpuseDataId +
              ');"><img src="' +
              script.pluginsUrl +
              '/speedship-myunishippers-rating/warehouse-dropship/wild/assets/images/delete.png"></a></td>'
