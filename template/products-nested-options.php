    ...
    class EnUpdateProductNestedOptions
    {
        ...
        public function en_add_nested_simple_product_hooks()
        {
            ...
            add_action(
                ...
                array($this, 'en_show_nested_product_fields'),
                110
            );
            add_action(
                'woocommerce_process_product_meta',
                array($this, 'en_save_nested_product_fields'),
                20
            );
        }
        ...
        function en_save_nested_product_fields($post_id)
        {
            ...
            if (isset($post_id) && $post_id > 0) {
                ...
                $nestedMaterial = isset($_POST['_nestedMaterials' . $post_id]) ? sanitize_text_field($_POST['_nestedMaterials' . $post_id]) : '';
                update_post_meta($post_id, '_nestedMaterials', esc_attr($nestedMaterial));

                //              add / update Nested Percentage
                $nestedPercentage = isset($_POST['_nestedPercentage' . $post_id]) && !empty($_POST['_nestedPercentage' . $post_id]) ? sanitize_text_field($_POST['_nestedPercentage' . $post_id]) : 0;

                update_post_meta($post_id, '_nestedPercentage', esc_attr($nestedPercentage));

                //              add / update Nested Dimension
                $nestedDimension = sanitize_text_field($_POST['_nestedDimension' . $post_id]);
                update_post_meta($post_id, '_nestedDimension', esc_attr($nestedDimension));

                //              add / update Max. Nested Items
                $maxNestedItems = sanitize_text_field($_POST['_maxNestedItems' . $post_id]);
                update_post_meta($post_id, '_maxNestedItems', esc_attr($maxNestedItems));
                //              add / update Max. Nested Items
                $nestedStakingProperty = sanitize_text_field($_POST['_nestedStakingProperty' . $post_id]);
            ...
            if (!empty($plan_notifi) && (isset($plan_notifi['nested_material']))) {
                ...
                if (strlen($disable_plugins) > 0) {
                    if (strlen($enable_plugins) > 0) {
                        ...
                        // $description = apply_filters('speed_wwe_small_packages_quotes_plans_notification_link', array(3));
